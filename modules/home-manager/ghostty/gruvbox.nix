...
let
  cfg = config.nixporn;
  inherit (cfg.colorschemes) gruvbox;
  inherit (gruvbox.palette) ansi;
  target = "ghostty";
  enable = cfg.enable && cfg.colorscheme == "gruvbox" && cfg.${target}.enable;
  theme = pkgs.writeText "gruvbox-ghostty" ''
    background = ${ansi.bg}
    foreground = ${ansi.fg}
...
in
{
  config = lib.mkIf enable {
    xdg.configFile."ghostty/themes/gruvbox".source = theme;
    programs.ghostty.settings.theme = "light:gruvbox,dark:gruvbox";
  };
}
