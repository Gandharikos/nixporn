...
static_source = Path(argv[2])
animated_source = Path(argv[3])
output = Path(argv[4])
canvas_size = int(argv[5])
...
for name, settings in cursors.items():
    ...
    static_image = static_source / f"{name}.svg"
    animated_images = sorted(animated_source.glob(f"{name}-*.svg"))
    images = [static_image] if static_image.exists() else animated_images

    if not images:
        raise FileNotFoundError(f"missing SVG source for {name}")
