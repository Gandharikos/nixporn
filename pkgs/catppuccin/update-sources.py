...
async def main():
    ...
    parser = argparse.ArgumentParser(prog="update-sources")
    parser.add_argument("ports", default=cur_sources.keys(), nargs="*")
    parser.add_argument("-r", "--remove", action="store_true")
    args = parser.parse_args()

    assert not args.remove or len(args.ports) > 0, "Ports must be provided when passing --remove"

    new_sources = cur_sources.copy()
    await asyncio.gather(*[handle_port(new_sources, port, remove=args.remove) for port in args.ports])
